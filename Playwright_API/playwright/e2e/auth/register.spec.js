...
test.describe("POST /auth/register", () => {
    ...
    //Cria um hook para proximas execuções, uma vez que se repete sempre
    test.beforeEach(({request})=>{
        auth = authService(request);
    })

  test("Deve cadastrar novo usuário | 200", async ({ request }) => {
  ...
  test("Não deve cadastrar com email existente | 200", async ({ request }) => {
  ...
  test("Não deve cadastrar com nome vazio", async ({ request }) => {
    const user = {
      name: "", //nome vazio
  ...
  test("Não deve cadastrar com senha vazio", async ({ request }) => {
    const user = {
  ...
  test("Não deve cadastrar com email vazio", async ({ request }) => {
    const user = {
      name: "Felipe Pnehas",
      email: "", //email vazio
      password: "Pesta",
